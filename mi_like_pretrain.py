...
import models_mae
from smi import compute_and_log_mi, make_projection_matrices
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/tmp/datasets/imagenet100', type=str,
                        help='dataset path')
    ...
    parser.add_argument('--bt_variant', type=str, default="per_batch")
    parser.add_argument('--bt_weight', type=float, default=0.05)
    parser.add_argument('--bt_lambda', default=0.005, type=float)
...
def main(args):
    ...
    val_loader_main_proc = torch.utils.data.DataLoader(
        dataset_val,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
    ...
    U_list, V_list = make_projection_matrices(
        # dX=model_without_ddp.embed_dim,
        # dY=model_without_ddp.embed_dim,
        dX=768,
        dY=768,
        proj_k=5,
        n_proj=1000,
    ...
    mask1_fixed = torch.rand(1, model_without_ddp.patch_embed.num_patches, device=device)
    mask2_fixed = torch.rand(1, model_without_ddp.patch_embed.num_patches, device=device)
